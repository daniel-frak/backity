...
class SourceFileHttpDtoMapperTest {

    private static final SourceFileHttpDtoMapper MAPPER = Mappers.getMapper(SourceFileHttpDtoMapper.class);

    @Test
    void shouldMapToDto() {
        SourceFile domain = TestSourceFile.gog();

        SourceFileHttpDto result = MAPPER.toDto(domain);

        SourceFileHttpDto expectedResult = dto();
        assertThat(result)
                .usingRecursiveComparison().isEqualTo(expectedResult);
    }

    private SourceFileHttpDto dto() {
        return new SourceFileHttpDto(
