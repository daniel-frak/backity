...
class GetDiscoveredFileListUseCaseTest {
    ...
    @BeforeEach
    void setUp() {
        useCase = new GetDiscoveredFileListUseCase(gameFileDetailsRepository);
    }

    @Test
    void shouldGetDiscoveredFileList() {
        Pagination pagination = new Pagination(0, 10);
        Page<GameFileDetails> gameFileDetails = new Page<>(singletonList(inProgressFileDetails().build()),
                1, 1, 1, 10, 0);
        when(gameFileDetailsRepository.findAllDiscovered(pagination))
                .thenReturn(gameFileDetails);

        Page<GameFileDetails> result = useCase.getDiscoveredFileList(pagination);
