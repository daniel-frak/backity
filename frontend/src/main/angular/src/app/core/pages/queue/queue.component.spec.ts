...
describe('QueueComponent', () => {
    ...
    beforeEach(async () => {
        fileCopiesClient = {
            getFileCopyQueue: vi.fn(),
            cancelFileCopy: vi.fn()
        } as unknown as Mocked<FileCopiesClient>;
        storageSolutionsClient = {
            getStorageSolutionStatuses: vi.fn()
        } as unknown as Mocked<StorageSolutionsClient>;
        messageService = {
            watch: vi.fn()
        ...
        fileCopiesClient.getFileCopyQueue.mockReturnValue(of({ content: [] }) as any);
    ...
    it('should show failure notification given error when refresh is called', fakeAsync(() => {
        ...
        fileCopiesClient.getFileCopyQueue.mockReturnValue(throwError(() => mockError));

        fixture.detectChanges(); // triggers ngOnInit → refresh
        tick();

        expect(notificationService.showFailure).toHaveBeenCalledWith('Error fetching enqueued files', mockError);
    ...
    it('should reload files and storage solution statuses when refresh is called', fakeAsync(() => {
        fileCopiesClient.getFileCopyQueue.mockReturnValue(of(deepClone(initialQueue) as any));
        storageSolutionsClient.getStorageSolutionStatuses.mockReturnValue(of(deepClone(initialStorageSolutionStatusResponse) as any));
    ...
    it('should get storage solution status', async () => {
        ...
        const result: StorageSolutionStatus | undefined = component.getStorageSolutionStatus("someStorageSolutionId");
    ...
    it('should remove file copy from queue when status changed event is received' +
        ...
        simulateFileCopyStatusChangedEventReceived(initialEnqueuedFileCopy.id, initialEnqueuedFileCopy.naturalId, FileCopyStatus.StoredIntegrityVerified);
    ...
    it('should not remove file copy from queue when status changed event is received' +
        ...
        component.fileCopyWithContextPage.set(TestPage.of([initialFileCopyWithContext]));
        simulateFileCopyStatusChangedEventReceived(initialEnqueuedFileCopy.id, initialEnqueuedFileCopy.naturalId, FileCopyStatus.Enqueued);
    ...
    it('should not remove file copy from queue when status changed event is received' +
        ...
        component.fileCopyWithContextPage.set(TestPage.of([initialFileCopyWithContext]));
        simulateFileCopyStatusChangedEventReceived(initialEnqueuedFileCopy.id, initialEnqueuedFileCopy.naturalId, FileCopyStatus.InProgress);
    ...
    it('should not remove file copy from queue when status changed event is received and page is undefined', () => {
        ...
        emitFileCopyStatusChanged(TestFileCopyStatusChangedEvent.withContent('any', { sourceFileId: 'any', backupTargetId: 'any' }, FileCopyStatus.StoredIntegrityVerified));
    ...
    it('should not remove file copy from queue when status changed event is received and item' +
        ...
        emitFileCopyStatusChanged(TestFileCopyStatusChangedEvent.withContent(item.fileCopy.id, item.fileCopy.naturalId, FileCopyStatus.StoredIntegrityVerified));
    ...
    it('should not remove file copy from queue when status changed event is received but file copy not found', fakeAsync(() => {
        fixture.detectChanges();
        tick();
        ...
        simulateFileCopyStatusChangedEventReceived(notFoundFileCopy.id, notFoundFileCopy.naturalId, FileCopyStatus.InProgress);
    ...
    it('should update file copy progress in queue when progress update event is received' +
        ' given file copy found', fakeAsync(() => {
        fixture.detectChanges();
        tick();

        component.fileCopyWithContextPage.set(TestPage.of([initialFileCopyWithContext]));
        simulateReplicationProgressUpdateEventReceived(initialEnqueuedFileCopy.id, initialEnqueuedFileCopy.naturalId);

        expect(fixture.nativeElement.textContent).toContain('25%');
    }));

    function simulateReplicationProgressUpdateEventReceived(fileCopyId: string, fileCopyNaturalId: FileCopyNaturalId): void {
        const event: FileCopyReplicationProgressUpdatedEvent = TestProgressUpdatedEvent.twentyFivePercent(fileCopyId, fileCopyNaturalId);
    ...
    it('should not update file copy progress in queue when progress update event is received' +
        ' given file copy not found', fakeAsync(() => {
        fixture.detectChanges();
        tick();

        component.fileCopyWithContextPage.set(initialQueue);
        simulateReplicationProgressUpdateEventReceived('unknownFileCopyId', { sourceFileId: 'unknownSourceFileId', backupTargetId: 'unknownBackupTargetId' });
    ...
    it('should not update file copy progress in queue when progress update event is received' +
        ' given page is undefined', fakeAsync(() => {
        fixture.detectChanges();
        tick();
    ...
    it('cancelBackup should cancel backup and refresh', fakeAsync(() => {
        ...
        fileCopiesClient.getFileCopyQueue.mockReturnValue(of(initialQueue) as any);
    ...
    it('should log error when cancelling file copy backup fails', fakeAsync(() => {
        ...
        expect(notificationService.showFailure).toHaveBeenCalledWith(`An error occurred while trying to cancel the backup`, fileCopyWithContext, mockError);
    ...
    it('should not update file copy status in queue when status changed event is received' +
        ...
        emitFileCopyStatusChanged(TestFileCopyStatusChangedEvent.withContent('any', { sourceFileId: 'any', backupTargetId: 'any' }, FileCopyStatus.InProgress));
    ...
    it('should not update file copy status in queue when status changed event is received' +
        ...
        emitFileCopyStatusChanged(TestFileCopyStatusChangedEvent.withContent(item.fileCopy.id, item.fileCopy.naturalId, FileCopyStatus.InProgress));
    ...
    it('should block refresh when already loading', async () => {
        fileCopiesClient.getFileCopyQueue.mockClear();
        component.fileCopiesAreLoading.set(true);
