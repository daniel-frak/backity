...
describe('EditBackupTargetModalComponent', () => {
    ...
    beforeEach(async () => {
        ...
        backupTargetsClient = TestBed.inject(BackupTargetsClient) as any;
    ...
    function editingBackupTargetSucceeds() {
        backupTargetsClient.editBackupTarget.mockReturnValue(of({}) as any);
    }
    ...
    it('should gracefully handle errors when editing backup target fails', async () => {
        ...
        expect(notificationService.showFailure).toHaveBeenCalledWith("Something went wrong when editing a Backup Target.", error);
        expect(modal.timesClosed).toBe(0);
        expect(component.isLoading()).toBe(false);
    });

    function editingBackupTargetThrows(error: Error) {
        backupTargetsClient.editBackupTarget.mockReturnValue(throwError(() => error));
    }
});
