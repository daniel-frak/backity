...
describe('ModalServiceStub', () => {
    ...
    it('should throw when getting a modal that is not registered', () => {
        expect(() => service.getModalRef(DummyModalComponent)).toThrowError(/No ModalRefMock registered/);
    });
    ...
    it('should throw when opening an unregistered modal', () => {
        expect(() => service.open(DummyModalComponent)).toThrowError(/No ModalRefMock registered/);
    });
    ...
    it('withConfirmationModal should propagate errors thrown in the callback', async () => {
        ...
        await expect(service.withConfirmationModal('Confirm?', callback)).rejects.toEqual(error);
    });
});
